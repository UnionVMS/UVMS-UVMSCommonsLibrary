...
public abstract class AbstractRemoteProducer implements MessageProducer {
	...
	private void initializeProperties() {
		...
		textMessageProperties = new TextMessageProperties(FLUX_ENV_AD_VAL, FLUX_ENV_DF_VAL, FLUX_ENV_AR_VAL,
				FLUX_ENV_FR, createBusinessUUID(), createStringDate());
	}
	...
	/**
	 * Prepare the message for sending and set minimal set of attributes, required
	 * by FLUX TL JMS to understand how it should process it;
	 *
	 * @param textMessage
	 * @return fluxMsg
	 * @throws JMSException
	 ...
	 */
	protected TextMessage prepareMessage(final String textMessage, final Session session)
			throws JMSException, ServiceException {
	...
	private Connection openRemoteConnection() throws NamingException, JMSException {
		...
		try {
			...
			connection = connectionFactory.createConnection(connectionProperties.getUsername(),
	...
	private Connection retryConnecting() throws JMSException {
		...
		Connection connection = null;
		try {
			connection = connectionFactory.createConnection(connectionProperties.getProviderURL(),
					connectionProperties.getPassword());
	...
	/**
	 * BUSINESS_UUID has a prefix, a date-time combination and a serial - thus it is
	 * semi unique
	 *
	 * @return randomUUID
	 */
